    ...
    return <div className="bg-white">
        <div className="mx-auto max-w-2xl px-4 pb-24 pt-16 sm:px-6 lg:max-w-7xl lg:px-8">
            ...
            <div className="mt-12 lg:grid lg:grid-cols-12 lg:items-start lg:gap-x-12 xl:gap-x-16">
                    <div className={cn("lg:col-span-7", {"rounded-lg border-2 border-dashed border-zince-200 p-12" : items.length === 0 , })}>
                                ...
                                return(
                                    <li key={product.id} className="flex py-6 sm:py-10">
                                        ...
                                        <div className="ml-4 flex flex-col justify-between sm:ml-6 ">
                                            ...
                                            <p className="mt-4 flex space-x-2 text-sm text-gray-900">
                    ...
                    <section className="mt-16 rounded-lg bg-gray-50 px-4 py-6 sm:p-6 lg:col-span-5 lg:mt-0 lg:p-8 ">
                            <h2 className="text-lg font-mediium text-gray-900">Order Summary</h2>
                            <div className="mt-6 space-y-4">
                                <div className="flex items-center justify-between">
                                    <p className="text-sm text-gray-600">Subtotal</p>
                                    <p className="text-sm font-medium text-gray-900">

                                        {isMounted ? formatPrice(cartTotal) : <span><Loader2 className="h-4 w-4 animate-spin"></Loader2></span>}
                                    </p>
                                </div>
                                ...
                                <div className="flex items-center justify-between border-t border-gray-200 pt-4">
                                    ...
                                    <div className="text-sm font-medium text-gray-900">
                                        {isMounted ? formatPrice(fee) :<Loader2 className="h-4 w-4 animate-spin"></Loader2> }
                                    </div>
                                </div>
                                <div className="flex items-center justify-between border-t border-gray-200">
                                    <div className="text-base font-medium text-gray-900">Order Total</div>
                                    <div className="text-base font-medium text-gray-900">{isMounted ? formatPrice(cartTotal + fee) :<Loader2 className="h-4 w-4 animate-spin"></Loader2> }</div>
                                </div>
                            </div>

                            <div className="mt-6">
                                <Button disabled={items.length===0 ||isLoading} className="w-full" size='lg' onClick={()=> createCheckoutSession({productIds})}>{isLoading ? <Loader2 className="w-4 h-4 animate-spin mr-1.5"/>: null}Checkout</Button>
                            </div>  
                    </section>
            </div>
        </div>
